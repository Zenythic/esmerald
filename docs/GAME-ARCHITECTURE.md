...
| 2026-07-07  | Estado de `olah` → **Demo funcional**. Juego tipo crash con temática marina: el barco "FORTUNA" remonta olas (tamaño aleatorio) y el multiplicador sube de forma continua; el jugador decide cuándo detenerse. Hundimiento provably-fair por umbral de crash `C = clamp(RTP/u, 1, 50)`. RTP 95%, tope 50x (Gran Ola). Escena tropical con placeholders CSS/SVG (sprites pendientes). Integrado en la landing como ruta `/games/olah/`. | Zenythic  |
| 2026-07-08  | Estado de `trade` → **Demo funcional**. Juego tipo predicción de líneas: un gráfico de líneas sube y baja puntada a puntada y cada ciertas puntadas entra en pausa de 5s donde el jugador decide si la próxima puntada irá arriba o abajo. Acertar paga la ganancia neta (multiplicador = f(distancia)); fallar pierde solo la MITAD de esa ganancia y el juego continúa. Sesión continua con stake comprometido; salida voluntaria en cualquier pausa. RTP 94% cerrado sesgando la probabilidad de acierto por puntada `p(m)=(1−0.12/(m−1))/3` (no degradando el multiplicador). MIN_MULT 1.20 / MAX_MULT 3.00. Gráfico SVG de líneas. Integrado en la landing como ruta `/games/trade/`. | Zenythic  |
